...
class AddFlavourScreen extends StatefulWidget {
  ...
  const AddFlavourScreen({super.key, required this.itemBloc});
...
class _AddFlavourScreenState extends State<AddFlavourScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder(
        stream: widget.itemBloc.itemFlavourState,
        builder: (context, snapshot) {
          if (widget.itemBloc.itemFlavourState.value.isLoading()) {
            return const DefaultLoadingWidget();
          } else if (widget.itemBloc.itemFlavourState.value.isError()) {
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Visibility(
                    visible: widget.itemBloc.resItemFlavourModel.data?.list?.isEmpty ?? true,
                    child: Text("No data available".i18n),
                  ),
                  StreamBuilder(
                    stream: widget.itemBloc.itemFlavourValue,
                    builder: (context, snapshot) {
                      return ListView(
                        shrinkWrap: true,
                        physics: const NeverScrollableScrollPhysics(),
                        children: widget.itemBloc.resItemFlavourModel.data!.list!.map((e) {
                          return Padding(
                            padding: AppPaddingConstants().bottom25,
                            child: AppCircleRadioOption(
                              value: e,
                              groupValue: widget.itemBloc.itemFlavourValue.valueOrNull,
                              text: e.name!,
                              onChanged: (val) {
                                widget.itemBloc.itemFlavourValue.add(val);
                              },
                            ),
                          );
                        }).toList(),
  ...
  Future<void> showFlavourBottomSheet() async {
            ...
            AppButton(
              ...
              onPressed: () {
                ...
                if ((!nameError.value && nameController.text.isNotEmpty)) {
                  ...
                  widget.itemBloc.postItemFlavour(req);
                }
              },
            ),
            const SizedBox(
