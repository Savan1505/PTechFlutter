...
class AddVintageScreen extends StatefulWidget {
  final ItemBloc itemBloc;

  const AddVintageScreen({super.key, required this.itemBloc});

  @override
  State<AddVintageScreen> createState() => _AddVintageScreenState();
}

class _AddVintageScreenState extends State<AddVintageScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder(
        stream: widget.itemBloc.itemVintageState,
        builder: (context, snapshot) {
          if (widget.itemBloc.itemVintageState.value.isLoading()) {
            return const DefaultLoadingWidget();
          } else if (widget.itemBloc.itemVintageState.value.isError()) {
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Visibility(
                    visible: widget.itemBloc.resItemVintageModel.data?.list?.isEmpty ?? true,
                    child: Text("No data available".i18n),
                  ),
                  StreamBuilder(
                    stream: widget.itemBloc.itemVintageValue,
                    builder: (context, snapshot) {
                      return ListView(
                        shrinkWrap: true,
                        physics: const NeverScrollableScrollPhysics(),
                        children: widget.itemBloc.resItemVintageModel.data!.list!.map((e) {
                          return Padding(
                            padding: AppPaddingConstants().bottom25,
                            child: AppCircleRadioOption(
                              value: e,
                              groupValue: widget.itemBloc.itemVintageValue.valueOrNull,
                              text: e.name!,
                              onChanged: (val) {
                                widget.itemBloc.itemVintageValue.add(val);
                              },
                            ),
                          );
                        }).toList(),
  ...
  Future<void> showVintageBottomSheet() async {
            ...
            AppButton(
              ...
              onPressed: () {
                ...
                if ((!nameError.value && nameController.text.isNotEmpty)) {
                  ...
                  widget.itemBloc.postItemVintage(req);
                }
              },
            ),
            const SizedBox(
